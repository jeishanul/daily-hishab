        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    ...
                    @can('employee.create')
                        <a href="{{ route('employee.create') }}" class="btn common-btn">
                            ...
                            <tbody>
                                @foreach ($staffs as $staff)
                                    <tr>
                                        ...
                                        <td>{{ $staff->user->personalInfo->phone ?? 'N/A' }}</td>
                                        <td>{{ ucfirst($staff->user->roles[0]->name ?? 'N/A') }}</td>
                                        <td>{{ $staff->department->name ?? 'N/A' }}</td>
                                        <td>{{ $staff->user->personalInfo->country ?? 'N/A' }}</td>
                                        <td>{{ $staff->user->personalInfo->city ?? 'N/A' }}</td>
                                        <td>{{ $staff->user->personalInfo->address ?? 'N/A' }}</td>
