        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <x-section-header title="suppliers" class="text-white" />
                    <div>
                            ...
                            <tbody>
                                @foreach ($suppliers as $supplier)
                                    <tr>
                                        ...
                                        <td>
                                            <img src="{{ $supplier->media->file ?? asset('public/default/default.jpg') }}"
                                                height="30" width="30">
                                        </td>
                                        <td>{{ $supplier->name }}</td>
                                        <td>{{ $supplier->personalInfo->company_name ?? 'N/A' }}</td>
                                        <td>{{ $supplier->personalInfo->tax_no ?? 'N/A' }}</td>
                                        <td>{{ $supplier->email }}</td>
                                        <td>{{ $supplier->personalInfo->phone ?? 'N/A' }}</td>
                                        <td>{{ $supplier->personalInfo->country ?? 'N/A' }}</td>
                                        <td>{{ $supplier->personalInfo->state ?? 'N/A' }}</td>
                                        <td>{{ $supplier->personalInfo->city ?? 'N/A' }}</td>
                                        <td>{{ $supplier->personalInfo->address ?? 'N/A' }}</td>
                                        <td>{{ $supplier->personalInfo->zip_code ?? 'N/A' }}</td>
                                        <td>
                        ...
                        <div class="modal-body">
                            <div class="row mb-3">
                                <div class="col-md-12 mb-3">
                                    <x-fileInputGroup name="file" title="{{ __('import') }}" :required="true"
